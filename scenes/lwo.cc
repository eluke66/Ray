...
Scene* lwo()
{
  ...
  Object * lam = ObjectLoader::LoadObject( "Objects/Lotus Elise.lwo", "" );
  Position p;
  p.translate( Vector(-2,-2,3), true );
  p.rotate( Vector(270,135,0) );
  p.scale( 0.25, Position::SCALE_MAX );
  p.transform(lam);

  world->addObject( lam );

  {
    Object * lam = ObjectLoader::LoadObject( "Objects/Lamborghini_Diablo.lwo", "" );
    Position p;
    p.translate( Vector(-3,5,3.2), true );
