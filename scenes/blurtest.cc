...
Scene* blurtest()
{
  ...
  std::vector<Blur *> blurrers;

  blurrers.push_back( new ConstantBlur(0.0) );
  blurrers.push_back( new ConstantBlur(0.01) );
  blurrers.push_back( new GradientBlur( GradientColor(Color(0.1), Color( 0.0 ),
						      Color(0.005), Color( 0.01), 1., 1.)  ) );
  
  blurrers.push_back( new MapBlur( TextureColor( "check.jpg", 1.0, 1.0 ) ) );
  
  for(int i=0;i<4;i++){
    double eta = 2.;

    Material* transp_matl =
      new GlossyDielectricMaterial(eta, glass, 100,
				   blurrers[i]);
...

  {
    ...
    blurrers.push_back( new ConstantBlur(0.0) );
    blurrers.push_back( new ConstantBlur(0.1) );
    blurrers.push_back( new GradientBlur( GradientColor(Color(0.1), Color( 0.0 ),
							Color(0.005), Color( .1), 1., 1.)  ) );
    
    blurrers.push_back( new MapBlur( TextureColor( "check.ppm", 4.0, 4.0 ) ) );
    for ( int i = 0; i < 4; ++i ) {
      Material* ball_matl =
	new GlossyMetalMaterial(Color(0.8, 0.8, 0.8), 100,
				blurrers[i]);
      world->addObject(new SpherePolar(ball_matl, Point(-9 + 2.25*i, 3.5, 3.5),
				       1.0,
